...
namespace ProceduralMeshes.Generators
{
	public struct SharedTriangleGrid : IMeshGenerator
	{
		public void Execute<S> (int z, S streams) where S : struct, IMeshStreams
		{
			...
			{
				xOffset = .25f;
				uOffset = .5f / (Resolution + .5f);
				tA = int3(iA, iC, iB);
				tB = int3(iB, iC, iD);
			}
			xOffset = xOffset / Resolution - .5f;
			...
			vertex.position.z = ((float)z / Resolution - .5f) * sqrt(3f) / 2f;
			vertex.texCoord0.x = uOffset;
			vertex.texCoord0.y = vertex.position.z / (1f + 0.5f / Resolution) + .5f;
			...
			for (int x = 1; x <= Resolution; x++, vi++, ti += 2)
			{
				...
				vertex.texCoord0.x = x / (Resolution + .5f) + uOffset;
				streams.SetVertex(vi, vertex);
				if (z > 0)
				{
		...
		public Bounds Bounds => new Bounds(
			Vector3.zero, new Vector3(1f + .5f / Resolution, 0f, sqrt(3f) / 2f)
		);
	}
}
