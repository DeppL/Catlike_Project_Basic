...
public static class Shapes {
	...
	public struct Job<S> : IJobFor where S : struct, IShape {
		...
		public void Execute (int i) {
			...
			float3x4 n = transpose(normalTRS.TransformVectors(p.nornals, 0f));
			normals[i] = float3x4(
	...
	public struct Point4 {
		public float4x3 positions, nornals;
	}
	...
	public struct Torus : IShape {
		public Point4 GetPoint4(int i, float resolution, float invResolution) {
			...
			p.nornals = p.positions;
			p.nornals.c0 -= r1  * sin(2f * PI * uv.c0);
			p.nornals.c2 -= r1  * cos(2f * PI * uv.c0);
			return p;
		}
	}
}
